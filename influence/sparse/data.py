...
def data_to_sparse(data):
  """
  Converts a dense tensor of shape RxNxN to a list of R sparse tensors.
  
  Args:
  tensor: A dense tensor of shape (R, N, N) where R is the number of predicates 
          and NxN are the adjacency matrices.

  Returns:
  A list of R sparse tensors, each representing an NxN adjacency matrix.
  """
  R, N, _ = data.shape
  sparse_tensors = torch.Tensor(R, N, N)
  
  # Iterate over each predicate (R dimension)
  for r in range(R):
      dense_matrix = data[r]
      
      # Find the non-zero indices in the dense matrix
      indices = torch.nonzero(dense_matrix, as_tuple=True)
      values = dense_matrix[indices]
      
      # Stack the indices to form a 2xM tensor, where M is the number of non-zero elements
      indices = torch.stack(indices)
      
      # Create a sparse matrix for the current predicate
      sparse_tensor = torch.sparse_coo_tensor(indices, values, dense_matrix.size())
      
      torch.cat(sparse_tensors, sparse_tensor)
